...
| `device_friendly_name` | string | *(none)* | Purely cosmetic — shown alongside the device name in HA (e.g. "cj mac (Living Room MacBook)"), NOT used in any topic or unique_id. Without it, a kid who uses more than one Mac gets multiple HA devices that all display with the exact same name, with no way to tell them apart short of digging into each one's entities. Safe to change any time; doesn't affect entities. |
